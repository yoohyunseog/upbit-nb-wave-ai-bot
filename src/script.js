    ...
    function fetchDataAndUpdateChart() {
        $.ajax({
            ...
            success: function(data) {
                ...
                const summary = generateSummary(analyzedData, supportLevel, analyzedData.volumeNbMax, analyzedData.volumeNbMin, analyzedData.volumeUpDown);
                calculateAndAppendSum()

                $('#analysisSummary').html(summary);
...
function analyzeResistanceBreakthrough(currentPrice) {
    ...
    if (overallTrend === '매수세') {
        if (highestBuyPrice === 0 || currentPrice > highestBuyPrice) {
            ...
            //const Profit  = (totalResistanceCount * 0.002) + 1.000
            fallProfitPercent = -0.01;
            riseProfitPercent += 0.01;
          
            if(allNonZeroAndWithinLimit) {
                if(buyNbMaxSum_3_5 > buyNbMinSum_3_5 && sellNbMaxSum_6_8 < sellNbMinSum_6_8 && volumeUpDown === 'Up') {
                  executeBuyRequest();
        ...
        if (lowestSellPrice === 0 || currentPrice < lowestSellPrice) {
            ...
            if(allNonZeroAndWithinLimit) {
              if(buyNbMaxSum_3_5 < buyNbMinSum_3_5 && sellNbMaxSum_6_8 > sellNbMinSum_6_8 && volumeUpDown === 'Down') {
                executeSellRequest();
...
function generateSummary(analyzedData, supportLevel, volumeNbMax, volumeNbMin, volumeUpDown) {
    ...
    // 볼륨 분석 추가
    summary += `볼륨 분석 결과, 현재 N/B Max는 ${volumeNbMax}이며 N/B Min은 ${volumeNbMin}입니다. `;
...
function initializeArrays(count) {
  return {
    BIT_START_A50: new Array(count),
    BIT_START_A100: new Array(count),
    BIT_START_B50: new Array(count),
    BIT_START_B100: new Array(count),
    BIT_START_NBA100: new Array(count),
  };
}

function calculateBit(nb, bit = 5.5, reverse = false) {
  if (nb.length < 2) {
    nb.push(1, 2, 3, 4, 5, 6, 7, 8);
  }
  ...
  const CONT = 40;
  const miner = max - min;
  const plus = miner / (COUNT * nb.length - 1);
  const VIEW = BIT_NB / CONT;

  let { BIT_START_A50, BIT_START_A100, BIT_START_B50, BIT_START_B100, BIT_START_NBA100 } = initializeArrays(COUNT * nb.length);
  let count = 0;
  let averages = 0;
  ...
  for (let a = 0; a < nb.length; a++) {
    for (let i = 0; i < COUNT; i++) {
      const BIT_END = 1;
      const A50 = min + plus * (count + 1);
      const A100 = (count + 1) * BIT_NB / (COUNT * nb.length);
      const B50 = A50 - plus * 2;
      const B100 = A50 + plus;
      const NBA100 = A100 / (nb.length - BIT_END);

      BIT_START_A50[count] = A50;
      BIT_START_A100[count] = A100;
      BIT_START_B50[count] = B50;
      BIT_START_B100[count] = B100;
      BIT_START_NBA100[count] = NBA100;
      count++;
    }
    averages += nb[a];
  }
  ...
  if (reverse) {
    BIT_START_NBA100.reverse();
  }

  let NB50 = 0;
  for (let i = 0; i < nb.length; i++) {
    for (let a = 0; a < BIT_START_NBA100.length; a++) {
      if (BIT_START_B50[a] < nb[i] && BIT_START_B100[a] > nb[i]) {
        NB50 += BIT_START_NBA100[Math.min(a, BIT_START_NBA100.length - 1)];
        break;
      }
    }
  }
  ...
  averages = (averages / (nb.length * max)) * 100;
  NB50 = Math.min((NB50 / 100) * averages, BIT_NB);
...
$(document).ready(function() {
  ...
  function updateFiles(trend, callback) {
    ...
    $.getJSON(audioHref + 'audio-files.php?folder=' + folder, function(response) {
      if (response.error) {
        console.error(response.error); // 오류 메시지 출력
      } else {
        audioFiles = response.map(file => audioHref + decodeURIComponent(file));
        console.log('Loaded audio files:', audioFiles); // 디버깅을 위해 파일 경로 출력
    ...
    $.getJSON(imageHref + 'image-files.php?folder=' + folder, function(response) {
      if (response.error) {
        ...
        imageFiles = response.map(file => imageHref + decodeURIComponent(file));
        console.log('Loaded image files:', imageFiles); // 디버깅을 위해 파일 경로 출력
  ...
  function playRandomAudio() {
    if (audioFiles.length > 0) {
      var randomIndex = getRandomIndex(audioFiles.length);
      var audioSrc = audioFiles[randomIndex];
      console.log('Playing random audio:', audioSrc); // 디버깅을 위해 재생할 파일 경로 출력
  ...
  function updateRandomBackgroundImage() {
    if (imageFiles.length > 0) {
      ...
      console.log('Updating random image:', 'url("' + imageUrl + '")'); // 디버깅을 위해 이미지 경로 출력
  ...
  audioPlayer.addEventListener('ended', function() {
    if (audioFilesUpdated) {
      playRandomAudio();
      audioFilesUpdated = false; // Reset the update flag
    }
  });
