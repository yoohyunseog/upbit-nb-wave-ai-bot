                                    ...
                                    const modelY = model.circle.y;
                                        ...
                                        if (Math.abs(dx) > 1) {
                                          ...
                                          nbCoinItems.forEach((item, itemIndex) => {
                                            ...
                                            // 단, 카운트다운이 진행 중이거나 BTC 탐색 모드일 때는 덮어쓰지 않음
                                            if (!zoneDecision && !model.countdownStarted && !model.btcExplorationMode) {
                                                // 이전 상태와 다를 때만 로그 출력 (무한 로그 방지)
                                                if (targetAction !== '신호 대기') {
                                                 ...
                                                 // 의사결정에 따른 색상 변경
                                               if (targetAction === '매도') {
                                                   model.circle.setFillStyle(0xff8800); // 주황색 (매도)
                                                   ...
                                                   model.circle.setFillStyle(0x0088ff); // 파란색 (BTC 시장 방문 = 매수 준비)
                                                     if (window.logManager) {
                                                         window.logManager.addLog(`🔵 트레이너: BTC 시장 방문! 파란색으로 변경`);
                                                     }
                                               } else if (targetAction === 'N/B 길드 방문') {
                                                   model.circle.setFillStyle(0xff8800); // 주황색 (N/B 길드 방문 = 매도 준비)
                                                   ...
                                                   model.circle.setFillStyle(0xffff00); // 노란색 (대기)
                                                     if (window.logManager) {
                                                 ...
                                                 // 카운트다운 진행 중일 때는 신호 대기 센터로 이동 목표 설정
                                                 targetAction = '신호 대기';
                                                 model.targetAction = targetAction; // model에 저장
                                                 model.targetX = config.width / 2;
                                                 model.targetY = config.height / 2;
                                                           ...
                                                           if (window.nbGuildPolygon && blinkCount < maxBlinks) {
                                                               ...
                                                               if (window.nbGuildPolygon) {
                                                                   window.nbGuildPolygon.setFillStyle(originalColor);
                                                                                                     ...
                                                                                                     // 트레이너 객체에 지속적 진동 효과 추가
                                                   if (model.circle) {
                                                       // 트레이너 원형에 지속적 깜빡임 효과
                                                       const originalColor = model.circle.fillColor;
                                                       let blinkCount = 0;
                                                       const maxBlinks = 8; // 2초간 8번 깜빡임
                                        ...
                                        const dy = model.targetY - modelY;
                                                                                 ...
                                                                                 // 디버깅: 트레이너 상태 확인 (5초마다, 모든 상태에서)
                                         if (model.isTrainer && Math.floor(Date.now() / 1000) % 5 === 0) {
                                             const distanceToTarget = Math.sqrt((model.targetX - modelX) ** 2 + (model.targetY - modelY) ** 2);
                                             if (window.logManager) {
                                                 window.logManager.addLog(`🔍 트레이너 상태: targetAction=${targetAction}, 현재위치=(${Math.round(modelX)}, ${Math.round(modelY)}), 목표위치=(${Math.round(model.targetX)}, ${Math.round(model.targetY)}), 거리=${Math.round(distanceToTarget)}px, dx=${Math.round(dx)}, dy=${Math.round(dy)}`);
                                             }
                                         }
                                        ...
                                        if (Math.abs(dx) <= 2 && Math.abs(dy) <= 2) {
                                            ...
                                            // 일반적인 이동 로직 (이동 속도 개선)
                                            if (Math.abs(dx) > 0.5) {
                                                model.circle.x += dx * 0.1; // 속도 2배 증가
                                                model.name.x = model.circle.x;
                                                model.role.x = model.circle.x;
                                            }
                                            
                                            if (Math.abs(dy) > 0.5) {
                                                model.circle.y += dy * 0.1; // 속도 2배 증가
                                                model.name.y = model.circle.y - 6;
                                                model.role.y = model.circle.y + 6;
                                            }
                                        }
