...
async function loadTradingDashboard() {
    try {
        console.log('🪙 Loading Trading Dashboard...');
        return window.tradingDashboard.loadTradingDashboard();
    } catch (error) {
        console.error('❌ Failed to load Trading Dashboard:', error);
...
// 가격 차트 데이터로 그리기 (N/B Wave 오버레이 포함) - MOVED TO trading-dashboard.js
/*
...
function drawNbWaveScalpingLine(ctx, nbData, width, height, stepX, dataLength, minPrice, maxPrice, priceRange, chartPadding) {
    ...
    // NB Window 값 표시 (API에서 받은 값)
    const nbWindow = nbData.window || 50;
    const randomBitUsed = nbData.random_bit_used || (5.5 + (nbWindow % 95) * 0.5);
...
async function initializeTimeframeCards() {
    try {
        ...
        if (container && window.timeframeCards) {
            container.innerHTML = window.timeframeCards.constructor.generateHTML('timeframe-cards-container', true);
    ...
    try {
        ...
        if (tradeRes.ok && nbRes.ok) {
            ...
            if (tradeData.chart_data && tradeData.chart_data.prices) {
                window.tradingDashboard.drawPriceChartFromData(tradeData.chart_data, nbData);
            }
...
async function initializeTradingCharts() {
    try {
        ...
        await window.tradingDashboard.initializeTradingCharts();
    } catch (error) {
...
function updateTradingCurrentZone(nbData) {
    ...
    if (currentZoneElement && zoneStrengthElement) {
        ...
        let zoneStrength = 0; // 기본 강도
        
        // 과반수 구역 우선 사용
        if (window.currentMajorityZone) {
            if (window.currentMajorityZone === 'BLUE') {
                currentZoneType = 'Blue Zone';
                zoneStrength = window.currentMajorityZoneStrength || 0;
                console.log('🔵 Using BLUE majority zone, strength:', zoneStrength);
            } else if (window.currentMajorityZone === 'ORANGE') {
                currentZoneType = 'Orange Zone';
                zoneStrength = window.currentMajorityZoneStrength || 0;
                console.log('🟠 Using ORANGE majority zone, strength:', zoneStrength);
            } else {
                currentZoneType = 'Neutral Zone';
                zoneStrength = window.currentMajorityZoneStrength || 0;
                console.log('⚪ Using NEUTRAL majority zone, strength:', zoneStrength);
            }
            ...
            if (window.lastZoneFromChart === 'BLUE') {
                currentZoneType = 'Blue Zone';
                // 현재 구역 강도 변수 사용
                zoneStrength = window.currentZoneStrength || 0;
                console.log('🔵 Using BLUE zone from chart, strength:', zoneStrength, 'currentZoneStrength:', window.currentZoneStrength);
            } else if (window.lastZoneFromChart === 'ORANGE') {
                currentZoneType = 'Orange Zone';
                // 현재 구역 강도 변수 사용
                zoneStrength = window.currentZoneStrength || 0;
                console.log('🟠 Using ORANGE zone from chart, strength:', zoneStrength, 'currentZoneStrength:', window.currentZoneStrength);
            }
            ...
            if (lastZone.zone) {
                if (lastZone.zone === 'BLUE') {
                    currentZoneType = 'Blue Zone';
                    zoneStrength = lastZone.strength ? lastZone.strength * 100 : 0;
                    console.log('🔵 Last point is BLUE zone (fallback)');
                } else if (lastZone.zone === 'ORANGE') {
                    currentZoneType = 'Orange Zone';
                    zoneStrength = lastZone.strength ? lastZone.strength * 100 : 0;
                    console.log('🟠 Last point is ORANGE zone (fallback)');
                } else {
                    console.log('⚪ Last point is NEUTRAL zone (fallback)');
                }
            }
        } else {
        ...
        console.log('🎯 Current zone type:', currentZoneType, 'from chart:', window.lastZoneFromChart, 'strength:', zoneStrength);
        
        // 구역 표시 업데이트
        currentZoneElement.textContent = currentZoneType;
        
        // 구역별 색상 적용
        currentZoneElement.className = 'current-zone';
        ...
        // 구역 강도 업데이트 (현재 구역 강도 변수 사용)
        zoneStrengthElement.textContent = `강도: ${zoneStrength}`;
        
        // 우측 패널 업데이트
        if (rightCurrentZoneElement) {
            rightCurrentZoneElement.textContent = currentZoneType;
        ...
        console.log('🎯 Trading current zone updated:', currentZoneType, 'Strength:', zoneStrength);
...
// Selected Coin Status 업데이트
async function updateSelectedCoinStatus(selectedCoin, currentPrice) {
    try {
        console.log('🔄 Updating Selected Coin Status for:', selectedCoin);
        
        // 현재가가 0이면 다시 가져오기
        if (!currentPrice || currentPrice <= 0) {
            console.log('⚠️ Current price is 0, fetching fresh data...');
            const priceResponse = await fetch(`/api/trading-data?coin=${selectedCoin}`);
            const priceData = await priceResponse.json();
            
            if (priceData && priceData.data && priceData.data.length > 0) {
                currentPrice = priceData.data[priceData.data.length - 1].close || 0;
            } else if (priceData && priceData.current_price) {
                currentPrice = priceData.current_price;
            }
            
            console.log('📊 Fresh current price:', currentPrice);
        }
        
        // 저장된 지갑 데이터 사용 (API 호출 대신)
        const walletData = window.sharedWalletData;
        ...
        console.log('💰 Using shared wallet data:', walletData);
        ...
        // 코인 보유량 및 평균 매수가 업데이트
        const coinBalanceElement = document.getElementById('selected-coin-balance');
        let coinBalance = 0;
        let avgBuyPrice = 0;
        if (walletData && walletData.balances) {
            const coinData = walletData.balances.find(b => b.currency === selectedCoin);
            if (coinData) {
                coinBalance = parseFloat(coinData.balance) || 0;
                avgBuyPrice = parseFloat(coinData.avg_buy_price) || 0;
            }
        }
        if (coinBalanceElement) {
            coinBalanceElement.textContent = coinBalance.toFixed(8);
        }
        ...
        if (coinPriceElement) {
            coinPriceElement.textContent = `₩${currentPrice.toLocaleString()}`;
        }
        ...
        if (coinPnlElement) {
            ...
            const pnlText = `수익율: ${pnlPercent >= 0 ? '+' : ''}${pnlPercent.toFixed(2)}%`;
            coinPnlElement.textContent = pnlText;
            coinPnlElement.className = `status-card coin-pnl ${pnlPercent >= 0 ? 'positive' : 'negative'}`;
        }
        
        // 평균단가 업데이트
        const coinAvgPriceElement = document.getElementById('selected-coin-avg-price');
        if (coinAvgPriceElement) {
            coinAvgPriceElement.textContent = `평균단가: ${avgBuyPrice > 0 ? `₩${avgBuyPrice.toLocaleString()}` : '거래 내역 없음'}`;
        }
        
        console.log('✅ Selected Coin Status updated successfully');
        
    } catch (error) {
        console.error('❌ Failed to update Selected Coin Status:', error);
    }
}
...
function updateTimeframeZones() {
    ...
    const individualZonesHtml = window.timeframeResults.map(result => {
        ...
        return `
            <div class="timeframe-status-card ${isBlue ? 'blue-zone' : isOrange ? 'orange-zone' : 'neutral-zone'}" 
                ...
                <div class="timeframe-name">${result.name}</div>
    ...
    const zonesHtml = `
        <div class="timeframe-zone-summary">
            <div class="current-zone-display ${currentZone.toLowerCase()}-zone" id="current-zone-display" onclick="selectCurrentZone()">
    ...
    setTimeout(() => {
        ...
        // 초기 선택 (1D)
        const defaultTimeframe = window.timeframeRotationList[window.currentRotationIndex];
...
window.selectTimeframe = function(timeframe, dominantZone, zoneCount, totalZones, isAutoRotation = false) {
    ...
    if (timeframeDisplay) {
        const displayTimeframe = convertTimeframeToDisplay(timeframe);
...
function updateCurrentZoneDisplay(timeframe, zone, count, total) {
    ...
    if (currentZoneDisplay) {
        ...
        if (zoneStats) {
            ...
            if (zoneCountSpan) {
                zoneCountSpan.textContent = `${count}/${total} (${((count/total)*100).toFixed(1)}%)`;
            }
        }
        
        // Current Zone 배경색도 업데이트
        currentZoneDisplay.className = `current-zone-display ${zone.toLowerCase()}-zone`;
    }
}
