            ...
            <div class="card border-secondary rounded-3 p-3">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <span id="currentTimeframe" class="badge bg-primary fs-6">1h</span>
...
window.selectTimeframe = function(timeframe, dominantZone, zoneCount, totalZones, isAutoRotation = false) {
    ...
    // 모든 카드에서 선택 상태 제거 및 "this" 표시 제거
    const allCards = document.querySelectorAll('.timeframe-status-card');
    allCards.forEach(card => {
        card.classList.remove('selected');
        // "this" 표시 제거
        const thisIndicator = card.querySelector('.this-indicator');
        if (thisIndicator) {
            thisIndicator.remove();
        }
    });
    
    // 선택된 카드에 선택 상태 추가 및 "this" 표시 (수동 선택 시에만)
    const selectedCard = document.getElementById(`timeframe-card-${timeframe}`);
    if (selectedCard) {
        selectedCard.classList.add('selected');
        
        // 자동 순회가 아닐 때만 "this" 표시 추가
        if (!isAutoRotation) {
            const thisIndicator = document.createElement('div');
            thisIndicator.className = 'this-indicator';
            thisIndicator.textContent = 'this';
            thisIndicator.style.cssText = `
                position: absolute;
                top: 2px;
                right: 2px;
                background: #00ff00;
                ...
                z-index: 10;
            `;
            selectedCard.appendChild(thisIndicator);
        }
    }
...
function initializeDefaultTimeframe() {
    ...
    if (defaultCard) {
        ...
        thisIndicator.className = 'this-indicator';
        thisIndicator.textContent = 'this';
        defaultCard.appendChild(thisIndicator);
