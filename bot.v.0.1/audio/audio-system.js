...
class AudioSystem {
    ...
    initializeSettings() {
        // 볼륨 설정 초기화 (스타크래프트 사운드에 맞게 조정)
        this.masterVolume = 0.5; // 마스터 볼륨을 50%로 설정
        this.soundVolumes = {
            click: 0.4,      // Select.wav - 클릭 사운드
            success: 0.3,    // Move.wav - 성공 사운드
            error: 0.3,      // Back.wav - 에러 사운드
            type: 0.2,       // Startup.wav - 긴 사운드이므로 볼륨 낮게
            sequence: 0.15   // 연속 재생 시 더 낮게
        };
        this.soundEnabled = {
            click: true,
            success: true,
    ...
    updateSettingsUI() {
        ...
        // 개별 볼륨
        Object.keys(this.soundVolumes).forEach(soundType => {
            const slider = document.getElementById(`${soundType}-volume`);
            const value = document.getElementById(`${soundType}-volume-value`);
            if (slider && value) {
                slider.value = this.soundVolumes[soundType] * 100;
                value.textContent = Math.round(this.soundVolumes[soundType] * 100) + '%';
            }
        });

        // 체크박스
        Object.keys(this.soundEnabled).forEach(soundType => {
            const checkbox = document.getElementById(`enable-${soundType}`);
            if (checkbox) {
                checkbox.checked = this.soundEnabled[soundType];
            }
        });
    }
    ...
    // 볼륨 계산 (마스터 볼륨과 개별 볼륨 적용)
    calculateVolume(soundType) {
        const masterVol = this.masterVolume || 0.5;
        const soundVol = this.soundVolumes[soundType] || 0.3;
        return masterVol * soundVol;
    }
    ...
    play(soundType) {
        ...
        // 개별 사운드 활성화 확인
        if (!this.soundEnabled[soundType]) {
            console.log(`🎵 ${soundType} sound disabled, skipping play`);
            return;
        }
...
// DOM 로드 시 오디오 시스템 초기화
document.addEventListener('DOMContentLoaded', function() {
    console.log('🎵 Initializing Audio System...');
    audioSystem = new AudioSystem();
    
    // 전역으로 오디오 시스템 노출 (DOM 로드 후)
