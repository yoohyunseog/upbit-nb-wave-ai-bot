...
class BTCMarketProfitCalculator {
    ...
    performBasicCalculation(buyProfitRateDisplay) {
        ...
        // buyProfitRateDisplay가 null이면 로그만 출력
        if (!buyProfitRateDisplay) {
            console.log(`📊 BTC 시장에서 기본 매수 전 예상 수익률 계산: ${buyProfitRate.toFixed(2)}%`);
            if (window.logManager) {
                window.logManager.addLog(`📊 BTC 시장에서 기본 매수 전 예상 수익률 계산: ${buyProfitRate.toFixed(2)}%`);
            }
        } else {
            // 매수 전 예상 수익률 표시 업데이트
    ...
    updateProfitRateDisplay(buyProfitRate, buyProfitRateDisplay, infoData) {
        // buyProfitRateDisplay가 null이면 로그만 출력
        if (!buyProfitRateDisplay) {
            console.log(`📊 매수 전 예상 수익률: ${buyProfitRate.toFixed(2)}%`);
            if (window.logManager) {
                window.logManager.addLog(`📊 매수 전 예상 수익률: ${buyProfitRate.toFixed(2)}%`);
            }
            return;
        }
