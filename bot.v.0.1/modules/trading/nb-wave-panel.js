...
class NbWavePanel {
    ...
    drawChart(nbData) {
        ...
        // 현재가 동기화: NB Wave 요약의 현재가를 우측 패널에 반영
        try {
            const price = nbData && nbData.summary ? nbData.summary.current_price : undefined;
            if (typeof price === 'number' && !isNaN(price)) {
                const rightPriceEl = document.getElementById('right-trading-current-price');
                if (rightPriceEl) rightPriceEl.textContent = `₩${price.toLocaleString()}`;
                const selectedPriceEl = document.getElementById('selected-coin-price');
                if (selectedPriceEl) selectedPriceEl.textContent = `₩${price.toLocaleString()}`;
            }
        } catch (e) {
            // ignore sync errors
        }
